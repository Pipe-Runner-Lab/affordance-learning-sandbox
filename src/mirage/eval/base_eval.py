...
PARQUET_FILENAME = "df.franka_cabinet_drawer_top_close.gzip"
...
def generate_equally_spaced_scopes(
    ...
    """
    scopes = [
        int(num_envs / num_simultaneous_agents)
...
class Trainer:
    ...
    def single_agent_train(self) -> None:
        ...
        ), "This method is not allowed for multi-agents"
    ...
    def single_agent_eval(self) -> None:
        ...
        for timestep in tqdm.tqdm(
            ...
            states_c = states.cpu().numpy()
            actions_c = actions.cpu().numpy()
            terminated_c = terminated.cpu().numpy()

            data = np.hstack(
                (
                    np.arange(self.env.num_envs).reshape(-1, 1),
                    np.ones((self.env.num_envs, 1)) * timestep,
                    states_c[:, -2].reshape(-1, 1),
                    actions_c,
                    terminated_c,
                )
            )
            ...
            if sequence_data is not None:
                sequence_data = np.vstack([sequence_data, data])
            else:
                sequence_data = data
        ...
        sequence_data.to_parquet(f"data/{PARQUET_FILENAME}")
    ...
    def multi_agent_train(self) -> None:
        ...
        assert (
            self.env.num_agents > 1
        ), "This method is not allowed for single-agent"
