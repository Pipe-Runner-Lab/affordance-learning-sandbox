...
class FrankaCabinetCameraTask(RLTask):
    def __init__(self, name, sim_config, env, offset=None) -> None:
        ...
        #     np.ones(
        #         (self.camera_width, self.camera_height, 3), dtype=np.float32
        #     )
        #     * -np.Inf,
        #     np.ones(
        #         (self.camera_width, self.camera_height, 3), dtype=np.float32
    ...
    def cleanup(self) -> None:
        ...
        # TODO: override observation buffer for camera data
        # self.obs_buf = torch.zeros(
        #     (self.num_envs, self.camera_width, self.camera_height, 3),
        #     device=self.device,
        #     dtype=torch.float,
        # )
