...
class FrankaBlockTask(RLTask):
    ...
    def set_up_scene(
        ...
        """Clones environments based on value provided in task config and
        applies collision filters to mask collisions across environments.

        Args:
            scene (Scene): Scene to add objects to.
            replicate_physics (bool): Clone physics using PhysX API for better
            performance
        """
        ...
        self.add_franka_to_stage()
        if self.num_props > 0:
            self.add_props_to_stage()
    ...
    def init_data(self) -> None:
        ...
        self.franka_local_grasp_pos = franka_local_pose_pos.repeat(
            (self._num_envs, 1)
        )
        self.franka_local_grasp_rot = franka_local_grasp_pose_rot.repeat(
            (self._num_envs, 1)
        )

        # TODO: Removed drawer related positions, validate
    ...
    def post_reset(self):
        ...
        * initial reset
        ...
        if self.num_props > 0:
            (
                self.default_prop_pos,
                self.default_prop_rot,
            ) = self._props.get_world_poses()
    ...
    def pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        * Convert actions to forces and apply on robot
        ...
        env_ids_int32 = torch.arange(
            self._frankas.count, dtype=torch.int32, device=self._device
        )
    ...
    def get_observations(self) -> dict:
        ...
        * to the RL policy
        ...
        ) = self._frankas._lfingers.get_world_poses(clone=False)
        (
    ...
    def is_done(self) -> None:
        ...
        * Returns a boolean tensor of shape (num_envs,) to indicate
    ...
    def add_props_to_stage(self) -> None:
        prop_cloner = Cloner()
        ...
        prop_cloner.clone(
            ...
            positions=np.array(
                prop_pos
            ),  # TODO: Validate drawer position removal
    ...
    def reset_idx(self, env_ids):
        ...
        dof_pos = torch.zeros(
            (num_indices, self._frankas.num_dof), device=self._device
        )
        dof_vel = torch.zeros(
            (num_indices, self._frankas.num_dof), device=self._device
        )
    ...
    def compute_franka_reward(
        ...
        finger_close_reward_scale: torch.Tensor,
        ...
        lfinger_dist = torch.abs(
            franka_lfinger_pos[:, 2] - drawer_grasp_pos[:, 2]
        )
        rfinger_dist = torch.abs(
            franka_rfinger_pos[:, 2] - drawer_grasp_pos[:, 2]
        )
        ...
        open_reward = (
            cabinet_dof_pos[:, 3] * around_handle_reward
            + cabinet_dof_pos[:, 3]
        )  # drawer_top_joint
        ...
        rewards = torch.where(
            cabinet_dof_pos[:, 3] > 0.01, rewards + 0.5, rewards
        )
        rewards = torch.where(
            cabinet_dof_pos[:, 3] > 0.2,
        ...
        # TODO: Prevent bad style of moving, check code
